...
export const deleteCartItem = createAsyncThunk('cart/deleteCartItem', async ({ userId, productId }) => {
  const response = await axios.delete(`http://localhost:4000/api/shop/cart/${userId}/${productId}`);
...
export const updateCartQuantity = createAsyncThunk(
  ...
  async ({ userId, productId, quantity }) => {
    const response = await axios.put('http://localhost:4000/api/shop/cart/update-cart', {
      userId,
      productId,
      quantity,
    });
